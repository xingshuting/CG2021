...
TextureMapping::TextureMapping() {
	...
	// init model
	_sphere.reset(new Model(modelPath,"sphere.obj"));
...
void TextureMapping::initBlendShader() {
	...
	const char* fragCode =
		...
		"	color = mix(texture(mapKd1, TexCoord), texture(mapKd2, TexCoord), material.blend)* vec4(ambient  + diffuse, 1.0);\n"
...
void TextureMapping::initCheckerShader() {
	...
	const char* fragCode =
		...
		"void main() {\n"
		"	float step = 1.0f / (material.repeat);\n"	
		"	int i =  int( TexCoord[0]  / step) ; \n"
		"	int j =  int( TexCoord[1]  / step) ; \n"
		"	if( (i + j) % 2 == 0)\n"
		"		color = vec4(material.color1, 1.0f);\n"
		"	else\n"	
		"		color = vec4(material.color2, 1.0f);\n"
